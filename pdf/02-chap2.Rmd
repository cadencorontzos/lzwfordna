...
mean_comp_time <- function(corpus){
  ...
  means <- corpus %>% 
    ...
    # mean_decomp= .000000001*sum(Decompression.Time)/num_files
    )
  return(means)
}
  ...
  ord <- 1
	for(corpus_pair in array){
		...
		means2 <- mean_comp_time(cp2)
		  		...
		  		df<- df %>%  add_row(comp_throughput = throughput(cp1),
...
compare_max_string_lengths <- function(array){
    ...
    comp = rbind(cp1,cp2)
		means1 <- mean_comp_time(comp)

		df<- df %>%  add_row(comp_throughput = throughput(comp),
		  hm_comp = means1$mean_comp,
		  size = corpus_pair[3]
  ...
  plot1<- df %>% 
    ...
    geom_col(show.legend = F)+
    labs(x="Max String Length", y = "Throughput (Mb/s)", title= "Throughput")
  plot2 <- df %>% 
    ggplot(aes(x = size, y = hm_comp, fill = size))+
    geom_col(show.legend = F)+
    labs(x="Max String Length", y = "Mean Compression Time (s)", title = "Average Compression Time")	
  plot1+plot2
}
...
If we just save `currentBlock + next_character` into a new variable, that will prevent doing the concatenation 2 more times.
