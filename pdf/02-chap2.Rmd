...
compare_max_string_lengths <- function(array){
  ...
  plot1<- df %>% 
    ...
    geom_col()+
    labs(x="Max String Length", y = "Throughput(bytes/s)")
  plot2 <- df %>% 
...
As stated before, there have been research papers about the possibility of using multiple indexed dictionaries for LZW, including Keerthy [@KeerthyMID]. 
...
```{r findlongestfromavg, warnings= F, messages = F, fig.cap = "Comparing the different ways of finding the longest run", echo = F, fig.height = 7}
compare_coprora(list(
...
```{r findlongestwmax, warnings= F, messages = F, fig.cap = "Comparing the different ways of finding the longest run starting at average with strings over max not accepted", echo = F, fig.height = 7}
compare_coprora(list(
...
```{r maxenforcedstats, warnings= F, messages = F, fig.cap = "Compression ratios before and after not allowing strings over max", fig.height = 0.5, echo = F}
...
```{r findlongestwpext, warnings= F, messages = F, fig.cap = "Comparing the different ways of finding the longest run with pext", echo = F, fig.height = 7}
compare_coprora(list(
