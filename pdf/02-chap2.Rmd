...
stats_csv <- function(filename){
  df <- read.csv(filename)
  knitr::kable(df, format="latex", booktabs=TRUE, caption = "F") %>% 
...
```{r dictionarytechniquecompfig, echo = F, fig.height=7}
...
```{r findlongestfrommaxfig, warnings= F, messages = F, fig.cap = "foo", echo = F, fig.height = 10}
compare_coprora(list(
