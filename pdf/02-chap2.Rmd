...
harmonic_mean <- function(corpus){
  ...
  means <- corpus %>% 
    reframe(
      harmonic_mean_comp= sum(Compression.Time)/num_files,
      harmonic_mean_decomp= sum(Decompression.Time)/num_files
    )
  return(means)
}
...
compare_coprora <- function(array, per_corpus = FALSE, x_axis = "Milestone"){
  ...
  ord <- 1
	for(corpus_pair in array){
		...
		if(per_corpus){
		  		means1 <- harmonic_mean(cp1)
		means2 <- harmonic_mean(cp2)
		  
		  		df<- df %>%  add_row(comp_throughput = sum(cp1$Original.File.Size)/(.000001*sum(cp1$Compression.Time)),
		  hm_comp = means1$harmonic_mean_comp,
		...
		df<- df %>% add_row(
		  comp_throughput =sum(cp2$Original.File.Size)/(.000001*sum(cp2$Compression.Time)),
		  hm_comp = means2$harmonic_mean_comp,
		  description = corpus_pair[3],
		  order = ord,
		  corpus = "Corpus2"
		)
		  ...
		  comb <- rbind(cp1,cp2)
		  		...
		  		df<- df %>%  add_row(comp_throughput = sum(comb$Original.File.Size)/(.000001*sum(comb$Compression.Time)),
...
compare_max_string_lengths <- function(array){
    ...
    comp = rbind(cp1,cp2)
		means1 <- harmonic_mean(comp)

		df<- df %>%  add_row(comp_throughput = sum(comp$Original.File.Size)/(.000001*sum(comp$Compression.Time)),
		  hm_comp = means1$harmonic_mean_comp,
		  size = corpus_pair[3]
...
Most graphs in this section will refer to throughput and average compression time. Throughput is defined as the number of bytes that the program processes per second. The higher the throughput, the more efficient the algorithm. The average compression time is taken as the time of compression divided by the total number of files.
...
stats_csv('data/stats/corpus_2_fixedeof_9066eef.csv')
```
-->
### Using Constants
<!--
...
As seen in Figure \@ref(fig:directmaplength10statsfig), the throughput is greater and the average compression time is less for a direct map dictionary with a max string length of 15 as opposed to a max string length of 10. This makes sense, because any string over the max requires an entry in a hashmap, which takes much more time than a dictionary access. Using a string length of over 15 is difficult because the amount of memory required increases exponentially.
