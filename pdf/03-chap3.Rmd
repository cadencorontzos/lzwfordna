...
The other tools that we choose to use are `gzip`, `bzip`, and `genozip`. Both `bzip` and `gzip` are open source, general compression tools. As mentioned earlier `genozip` is was creates specifically for DNA compression and the compression of other common filetypes in biological research [@genozip]. 
