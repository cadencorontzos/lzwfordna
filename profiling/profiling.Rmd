...
```{r, echo=F}
df11 = read.csv("./DNACorpus1Run1.csv")
df21 = read.csv("./DNACorpus2Run1.csv")
kable(df11, caption = "Corpus 1")
```

\pagebreak

```{r}
kable(df21, caption = "Corpus 2")
```
...
```{r, echo=F}
df12 = read.csv("./DNACorpus1Run2.csv")
df22 = read.csv("./DNACorpus2Run2.csv")
kable(df12, caption = "Corpus 1")
```

\pagebreak

```{r}
kable(df22, caption = "Corpus 2")
```
...
```{r, echo=F}
df13 = read.csv("./DNACorpus1Run3.csv")
df23 = read.csv("./DNACorpus2Run3.csv")
kable(df13, caption = "Corpus 1")
```


```{r}
kable(df23, caption = "Corpus 2")
```
...
```{r, echo=F}
df14 = read.csv("./DNACorpus1Run4.csv")
df24 = read.csv("./DNACorpus2Run4.csv")
kable(df14, caption = "Corpus 1")
```


```{r}
kable(df24, caption = "Corpus 2")
```
...
```{r}
num_datasets<-4
corpus_name <- function(corpus_number, run_number){
  paste("DNACorpus", as.character(corpus_number),  "Run", as.character(run_number), ".csv", sep= "")
}
add_run_number <- function(dataset, thread_num){
  dataset <-dataset %>% 
    mutate(
      run= thread_num
    )
...
for( i in 1:num_datasets){
  one_filename <- corpus_name(1, i)
  two_filename <- corpus_name(2, i)
  corpus_one_run <- add_run_number(read.csv(one_filename), i)
  corpus_two_run <-  add_run_number(read.csv(two_filename), i)
  corpus1 <- rbind(corpus_one_run, corpus1)
  corpus2 <- rbind(corpus_two_run, corpus2)
}
...
corpus1 %>% 
  ggplot(aes(x=run))+
  geom_line(aes(y=Compression.Time, color = "blue"), linetype="dotted")+
  geom_point(aes(y=Compression.Time, color = "blue"))+
  geom_line(height =1, aes(y=Decompression.Time, color= "red"), linetype="dotted")+
  geom_point(aes(y=Decompression.Time, color = "red"))+
  facet_wrap(~File.Name, scale = "free", ncol=3)+
  ggtitle("Time Changes for Corpus 1")
```
...
corpus2 %>% 
  ...
  geom_line(height =1, aes(y=Decompression.Time, color= "red"), linetype="dotted")+
  geom_point(aes(y=Decompression.Time, color = "red"))+
  facet_wrap(~File.Name, ncol = 4, scale = "free")+
  ggtitle("Time Changes for Corpus 2")
```
...
mean_corpus1<- harmonic_mean(corpus1)
mean_corpus2<- harmonic_mean(corpus2)
```

```{r}
mean_corpus1 %>% 
  ggplot(aes(x=run))+
  geom_line(aes(y=harmonic_mean_comp, color = "blue"), linetype="dotted")+
  geom_point(aes(y=harmonic_mean_comp, color = "blue"))+
    geom_line(aes(y=harmonic_mean_decomp, color="red"), linetype="dotted")+
  geom_point(aes(y=harmonic_mean_decomp, color = "red"))+
  scale_x_continuous(breaks = seq(1,4, 1))+
  ggtitle("Harmonic Means for Corpus 1")
```
...
mean_corpus2 %>% 
  ...
  geom_point(aes(y=harmonic_mean_decomp, color = "red"))+
  scale_x_continuous(breaks = seq(1,4, 1))+
  ggtitle("Harmonic Means for Corpus 2")
```
```